...
int _tmain(int argc, LPCWSTR argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		if (wcscmp(argv[i], _T("--help")) == 0)
		{
			...
			wprintf_s(_T("  EndPointController.exe -c                    Cycles through active playback devices\n"));
			wprintf_s(_T("\n"));
...
void enumerateOutputDevices(TGlobalState* state)
{
	...
	// Set next device as default (cycle)
	if (state->cycle)
	{
		// Get default device
		...
		for (int i = 1; i <= (int)count; i++)
		{
			...
			// Device ID
			LPWSTR strID = NULL;
			HRESULT hr = state->pCurrentDevice->GetId(&strID);
			if (!SUCCEEDED(hr))
			{
				continue;
			}
			...
			int deviceDefault = (state->strDefaultDeviceID != '\0' && (wcscmp(state->strDefaultDeviceID, strID) == 0));
			if (deviceDefault != 1)
			{
				continue;
			}

			int nextDeviceId = i + 1;
			if (nextDeviceId > (int)count)
			{
				nextDeviceId = 1;
			}

			state->hr = state->pDevices->Item(nextDeviceId - 1, &state->pCurrentDevice);
			if (SUCCEEDED(state->hr))
			{
				...
				if (SUCCEEDED(state->hr))
				{
					...
					hr = state->pCurrentDevice->OpenPropertyStore(STGM_READ, &pStore);
					if (SUCCEEDED(hr))
					{
						std::wstring friendlyName = getDeviceProperty(pStore, PKEY_Device_FriendlyName);
						wprintf_s(friendlyName.c_str());
					}
				}
